...
for x_car = 1:sx, y_car=1:sy, x_amb=1:sx, y_amb=1:sy, vx_amb in -vx:vx, vy_amb in 0:vy
     R=0
   # speeding penalty
...
    for x_car = 1:mdp.size_x, x_amb = 1:mdp.size_x, y_car = 1:mdp.size_y, y_amb = 1:mdp.size_y, vx_amb= -(mdp.size_vx):mdp.size_vx, vy_amb=0:mdp.size_vy
...
function POMDPs.transition(mdp::AmbulanceWorld,
  ...
  elseif a == :forward
    # compute next position
    # consider limiting range of vy for ambulance so it can't jump sideways
    cxp = min(car_x + 1, mdp.size_x) ; amb_xp = max(min(amb_x + vx_val, mdp.size_x), 1);
    amb_yp = min(amb_y + vy_val, mdp.size_y);
    amb_vxp = vx_val;
    amb_vyp = min(vy_val + 1, 3);
  elseif a == :zero
  ...
  elseif a == :leftf
    # compute next position
    # consider limiting range of vy for ambulance so it can't jump sideways
    cxp = min(car_x + 1, mdp.size_x) ; amb_xp = max(min(amb_x + vx_val, mdp.size_x), -mdp.size_x);
    amb_yp = min(amb_y + vy_val, mdp.size_y);
...
function POMDPs.reward(mdp::AmbulanceWorld, state::AmbulanceProblemState, action::Symbol, statep::AmbulanceProblemState )
 # if we've crashed, the state doesn't change
  ...
  #   return 0.0
  # if we've reached the hospital, the state doesn't change
  #elseif (state.x_amb in (ceil(mdp.size_x/2) +1 ):mdp.size_x ) && (state.y_amb == mdp.size_y)
  #   return 0.0
  #end
  r = 0.0
  ...
  for i = 1:n
    if posequal_allstate(state, reward_states[i])
      r += reward_values[i]
      a = reward_states[i]; b = reward_values[i];
      #println("We are in state $state and we match with state $a which has a value of $b")
    end
  end
  return r
end
...
function POMDPs.isterminal(mdp::AmbulanceWorld, s::AmbulanceProblemState)
    if (s.x_car == s.x_amb && s.y_car == s.y_amb) || ((s.x_amb in (ceil(mdp.size_x/2)+1):mdp.size_x) && (s.y_amb == mdp.size_y))
      return true
    else
      return false
    end
