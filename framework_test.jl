...
   s1.x_car = deepcopy(s2.x_car)
   s1.y_car = deepcopy(s2.y_car)
   #s1.vx_car = s2.vx_car
   #s1.vy_car = s2.vy_car
   s1.x_amb = deepcopy(s2.x_amb)
   s1.y_amb = deepcopy(s2.y_amb)
   s1.vx_amb = deepcopy(s2.vx_amb)
   s1.vy_amb = deepcopy(s2.vy_amb)
   #s1.done = s2.done
   s1
end
 
...
Base.hash(s::AmbulanceProblemState, h::UInt64 = zero(UInt64)) = hash(s.x_car, hash(s.y_car, hash(s.x_amb, hash(s.y_amb, hash(s.vx_amb, hash(s.vy_amb, h))))))
...
function AmbulanceWorld()
 sx::Int64=15 #size_x
 sy::Int64=15 #size_y
 vx::Int64=3 #size_vx (indexing for velocity starts at 0)
 vy::Int64=3 #size_vy
 rs::Vector{AmbulanceProblemState}=[]
 rv::Vector{Float64}=[] #reward values at goal state
 ...
 amb_length::Int64=1
  ...
  for x_car = 1:sx, y_car=1:sy, x_amb=1:sx, y_amb=1:sy, vx_amb = -vx:vx, vy_amb = 0:vy
     
     # init R
     R=0

     # going fast reward. applied no matter what
     R += 10*(sqrt(vx_amb^2 + vy_amb^2))
  ...
     # hospital states
     hosp_strt = ceil(sx/2)
     if (x_amb in (hosp_strt+1):sx) && (y_amb==sy)
       R += 1000
     end

     # if we have crashed
     if (x_car == x_amb) && (y_car in [y_amb-amb_length, y_amb, y_amb+amb_length])
         R -= 2000 
     end
  ...
  a = length(rs); b = length(rv); c = 15*15*15*15*7*4;
  println("The length of the reward vector is $a (should equal $b) which should be $c")

  return AmbulanceWorld(sx, sy, vx, vy, rs, rv, discount_factor, amb_length) #changed vx
end
...
    for x_car = 1:mdp.size_x, y_car = 1:mdp.size_y, x_amb = 1:mdp.size_x, y_amb = 1:mdp.size_y, vx_amb= -(mdp.size_vx):(mdp.size_vx), vy_amb = 0:mdp.size_vy
...
function POMDPs.transition(mdp::AmbulanceWorld,
  ...
  # OUR DUMMY TERMINAL STATE IS: (-1, -1, -1, -1, -100, -100)
 # cyp = car_y
  # if we've crashed, go to weird dummy terminal state
  if (car_x == amb_x) && (car_y in [amb_y-1, amb_y, amb_y+1])
    cxp = -1; cyp = -1; amb_xp= -1; amb_yp = -1;
    amb_vxp = -100; amb_vyp = -100;
  # if we've reached the hospital, go to weird dummy terminal state
  elseif (amb_x in ceil(sx/2)+1:sx) && (amb_y == sy)
    cxp = -1; cyp = -1; amb_xp= -1; amb_yp = -1;
    amb_vxp = -100; amb_vyp = -100;
  elseif a == :forward
    # compute next position
    # consider limiting range of vy for ambulance so it can't jump sideways
    cxp = min(car_x + 1, mdp.size_x); cyp = car_y; amb_xp = max(min(amb_x + vx_val, mdp.size_x), 1);
    amb_yp = min(amb_y + vy_val, mdp.size_y);
    amb_vxp = vx_val;
    amb_vyp = min(vy_val + 1, 3);
  elseif a == :zero
  ...
  elseif a == :reverse
    # compute next position
    # consider limiting range of vx for ambulance so it can't jump sideways
    cxp = min(car_x + 1, mdp.size_x); cyp = car_y; amb_xp = max(min(amb_x + vx_val, mdp.size_x), 1);
    amb_yp = min(amb_y + vy_val, mdp.size_y);
    amb_vxp = vx_val;
    amb_vyp = max(vy_val - 1, 0);
  elseif a == :rightf
    # compute next position
    # consider limiting range of vy for ambulance so it can't jump sideways
    cxp = min(car_x + 1, mdp.size_x); cyp = car_y; amb_xp = max(min(amb_x + vx_val, mdp.size_x), 1);
  ...
  elseif a == :leftf
    ...
    cxp = min(car_x + 1, mdp.size_x); cyp = car_y; amb_xp = max(min(amb_x + vx_val, mdp.size_x), 1);
    amb_yp = min(amb_y + vy_val, mdp.size_y);
    amb_vxp = max(vx_val - 1, -3);
    amb_vyp = vy_val;
  end

  probability[1] = 1.0
  neighbors[1].x_car = cxp;
  neighbors[1].y_car = cyp;
...
function POMDPs.reward(mdp::AmbulanceWorld, state::AmbulanceProblemState, action::Symbol, statep::AmbulanceProblemState )
  #if we've crashed or reached the hospital, the state doesn't change (we're in the weird terminal state)
  if state.x_car == -1 && state.x_amb == -1 && state.y_car == -1 && state.y_amb == -1 && state.vx_amb == -100 && state.vy_amb == -100
     return 0.0
  end
  #r = 0.0

  # OTHERWISE, collect rewards as expected 
  reward_states = mdp.reward_states
  reward_values = mdp.reward_values
  n= length(reward_states)

  for i = 1:n
    if Base.isequal(state, reward_states[i])
      return reward_values[i]
    end
  end
  print("PANICCC")
  print(state)
...
function POMDPs.isterminal(mdp::AmbulanceWorld, state::AmbulanceProblemState)
    if state.x_car == -1 && state.x_amb == -1 && state.y_car == -1 && state.y_amb == -1 && state.vx_amb == -100 && state.vy_amb == -100
      return true
    else
      return false
...
# pass in an integer depth, integer num iterations, float exp_const, tuple of ints corresponding to start state: s
function the_test(the_depth, num_iterations, exp_const, s)
    ...
    solver = MCTSSolver(n_iterations=num_iterations, depth=the_depth, exploration_constant=exp_const)

    # initialize the policy by passing in your problem and the solver
    policy = MCTSPolicy(solver, mdp)

    hist = HistoryRecorder()
    state= AmbulanceProblemState(s[1],s[2],s[3],s[4],s[5],s[6])
    r = simulate(hist, mdp, policy, state)
    println("Total discounted reward $r")
    hist.state_hist
    println(hist.state_hist)

    return (r,hist.state_hist)
