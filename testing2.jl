...
type AmbulanceWorld <: MDP{AmbulanceProblemState, Action} # Note that our MDP is parametarized by the state and the action
...
function POMDPs.rand(rng::AbstractRNG, space::ActionSpace, a::Action)
...
function POMDPs.transition(mdp::AmbulanceWorld,
  ...
  car_x = state.x_car; car_y = state.y_car; amb_x = state.x_amb; amb_y = state.y_amb;
  vx = state.vx_amb; vy = state.vy_amb;
  ...
  neigbhors[1].y_car = state.y_car
  neigbhors[1].x_amb = amb_xp
  neighbors[1].y_amb = amb_yp
  neigbhors[1].vx_amb = amb_vxp
...
function POMDPs.reward(mdp::AmbulanceWorld, state::AmbulanceProblemState, action::Action, statep::AmbulanceProblemState )
 # if we've crashed, the state doesn't change
